...
import { liquidTransactionBuilder } from '../../../types';
...
const SignSendZBC = () => {
  ...
  const handleChageCompleteMinutes = ({ target: { value } = {}} = {}) => {
    setCompleteminutes(value)
    resetDisplay(true);
  }
  ...
  const buildTransaction = () => {
    const txBytes = liquidTransactionBuilder({
      sender: { value: publicKey, type: 0 },
      recipient:{ value: recipient, type: 0 },
      fee: parseInt(fee),
      amount: parseInt(amount),
      completeMinutes: parseInt(completeMinutes),
    });
    return txBytes;
  };
  ...
  return (
    <div>
      <div style={{ textAlign: 'center' }}>
        ...
        <div>
          <h4>Compete Minutes:</h4>
          <input name="amount" onChange={handleChageCompleteMinutes} value={completeMinutes} />
        </div>
        <SignatureGenerator
          accountIndex={accountIndex}
