...
const BlockList = () => {
  ...
  const listBlocks = () => {
    zoobc.Account.getBalance({ value: 'ZBC_F5YUYDXD_WFDJSAV5_K3Y72RCM_GLQP32XI_QDVXOGGD_J7CGSSSK_5VKR7YML', type: 0 }).then(res => {
      console.log(res);
    });
    /*zoobc.Block
      .getBlocks({height: 0})
        ...
        <tbody>
          {blocks.length > 0 &&
            blocks.map((data, key) => {
              return (
                <tr key={key}>
                  <td onClick={() => onClickBlockId(data.block.id)}>{data.block.id}</td>
                  <td>{data.block.previousblockhash}</td>
                  <td onClick={() => onClickBlockHeight(data.block.height)}>{data.block.height}</td>
